...
export namespace Table {
	...
	export type GetIndexKey<T extends Table, Index extends T['indexes'][number] = T['indexes'][number]> = Remap<
		U.IntersectOf<Table.GetIndexKeyMap<T>[Index]>
	>;

	export type GetIndexCursorKey<T extends Table, Index extends T['secondaryIndexes'][number]> = Remap<
		Index extends string
			? Table.GetIndexKeyMap<T>['primaryIndex'] & Partial<U.IntersectOf<Table.GetIndexKeyMap<T>[Index]>>
			: Table.GetIndexKeyMap<T>['primaryIndex']
	>;
...
> {
	...
	constructor(public config: Config, middleware: Array<DkMiddlewareHandler> = []) {
		this.client = config.client;
		this.dkClient = new DkClient(config.client);

		this.dkClient.setDefaults(this.config.defaults);
		this.dkClient.setMiddleware(middleware);
